...
### 2. 管理员仪表盘
- 4 张统计卡片：用户总数 / 房产总数 / 账单总数 / 待处理告警
- 侧边栏：9 个管理菜单 (仪表盘/用户/房产/电表/账单/缴费/告警/工单/电价)
- 顶栏：系统名称 + 用户姓名 + 角色标签 + 通知图标 + 退出

### 3. 用户管理页
- 表格：ID / 用户名 / 姓名 / 角色 / 手机 / 邮箱 / 状态 / 操作
- 按钮：新增用户 / 编辑 / 重置密码 / 禁用
- 弹窗：新增/编辑表单 (用户名/姓名/角色/密码/手机/邮箱)

### 4. 账单查询页
- 过滤条件：状态 (待缴费/已缴费/逾期) + 账期 (YYYYMM)
- 表格：用电量 / 阶梯用量 / 阶梯费用 / 电费 / 滞纳金 / 状态
...
| 收费员 | `collector01` | `col123` | 查看账单、处理告警、工单回复 |
| 收费员 | `collector02` | `col123` | 同上 |
| 居民 | `resident01` | `res123` | 我的账单/缴费/工单/通知 |
| 居民 | `resident02` ~ `resident10` | `res123` | 同上 |
