...
CREATE OR REPLACE PROCEDURE sp_generate_monthly_bills(
    ...
    -- 电价配置（三个档位）
    v_price1       price_config.unit_price%TYPE;  -- 第一档单价
    v_price2       price_config.unit_price%TYPE;  -- 第二档单价
    v_price3       price_config.unit_price%TYPE;  -- 第三档单价
    v_limit1       price_config.upper_limit%TYPE; -- 第一档上限(200)
    v_limit2       price_config.upper_limit%TYPE; -- 第二档上限(400)

    -- 抄表读数
    v_prev_reading meter_reading.reading_value%TYPE;  -- 上期读数
    v_curr_reading meter_reading.reading_value%TYPE;  -- 本期读数

    -- 用电量
    v_total_usage  NUMBER(10,2);
    v_tier1_usage  NUMBER(10,2);  -- 第一档用量
    v_tier2_usage  NUMBER(10,2);  -- 第二档用量
    v_tier3_usage  NUMBER(10,2);  -- 第三档用量
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('目标账期: ' || v_bill_month);
    ...
    BEGIN
        ...
        SELECT unit_price INTO v_price2
        FROM price_config
        ...
        -- 第二档上限（修复：从 tier_no=2 读取上限值 400）
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('SP1 错误: 未找到有效电价配置，请先初始化 price_config 表');
            RAISE;
    END;

    DBMS_OUTPUT.PUT_LINE('电价配置: 第一档 0-' || v_limit1 || '度, '
                      || v_price1 || '元/度 | 第二档 '
                      || v_limit1 || '-' || v_limit2 || '度, '
                      || v_price2 || '元/度 | 第三档 '
                      || v_limit2 || '度以上, ' || v_price3 || '元/度');

    -- -----------------------------------------------------------------------
    -- 步骤3: 遍历所有电表，为每个电表生成账单
    -- -----------------------------------------------------------------------
    FOR rec IN c_meters LOOP

        -- 检查是否已存在该电表当月的账单（幂等性保证）
        -- 若已存在，跳过，避免重复生成
        ...
        IF v_due_count > 0 THEN
            DBMS_OUTPUT.PUT_LINE('  电表 ' || rec.meter_no || ' 账期 ' || v_bill_month || ' 账单已存在，跳过');
            CONTINUE;
        END IF;

        -- -------------------------------------------------------------------
        -- 步骤3a: 获取本期读数（上个月最后一天的累计读数）
        --   从 meter_reading 表中查找 reading_date ≤ 上月最后一天 的最新记录
        -- -------------------------------------------------------------------
        BEGIN
            ...
            FROM (
                ...
                WHERE  meter_id    = rec.meter_id
                  AND  reading_date <= v_last_month_end
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                -- 该电表在上月没有抄表记录
                -- 可能是新装电表 / SP3 未覆盖 / 数据缺失
                DBMS_OUTPUT.PUT_LINE('  电表 ' || rec.meter_no || ' 在 ' || v_bill_month || ' 无抄表记录，跳过');
                CONTINUE;
        END;

        -- -------------------------------------------------------------------
        -- 步骤3b: 获取上期读数（上上个月最后一天的累计读数）
        --   同样从 meter_reading 查找。
        --   若查不到（新装电表第一份账单），使用 meter.initial_reading
        -- -------------------------------------------------------------------
        DECLARE
        ...
        BEGIN
            ...
            FROM (
                ...
                FROM   meter_reading
                WHERE  meter_id     = rec.meter_id
                  AND  reading_date <= v_prev_month_end
                ORDER BY reading_date DESC
            )
            WHERE ROWNUM = 1;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                -- 没有上上月读数，使用电表初始读数作为基准
                v_prev_reading := rec.initial_reading;
        END;

        -- -------------------------------------------------------------------
        -- 步骤3c: 计算总用电量
        --   如果本期读数不大于上期读数（倒转/数据异常），用量设为0并记录
        -- -------------------------------------------------------------------
        v_total_usage := v_curr_reading - v_prev_reading;
        IF v_total_usage < 0 THEN
            DBMS_OUTPUT.PUT_LINE('  警告: 电表 ' || rec.meter_no
                || ' 读数倒转 (prev=' || v_prev_reading
                || ', curr=' || v_curr_reading || ')，用量设为0');
            v_total_usage := 0;
        END IF;
        ...
        -- 算法:
        --   第一档用量 = MIN(总用量, 第一档上限)
        --   第二档用量 = MAX(0, MIN(总用量 - 第一档上限, 第二档上限 - 第一档上限))
        --   第三档用量 = MAX(0, 总用量 - 第二档上限)
        --
        --   各档费用 = 各档用量 × 各档单价
        --   总费用   = 各档费用之和
        -- -------------------------------------------------------------------
        ...
        -- -------------------------------------------------------------------
        -- 步骤3e: 确定缴费截止日 = 当前日期 + 15天
        --   (实际场景是出账日+15天，此处以过程执行日期为出账日)
        -- -------------------------------------------------------------------
        ...
        --   使用序列显式提供 bill_id
        --   触发效果:
        --     - TR4b (复合触发器) 会在语句结束后检测用电飙升/骤降
        --     - 如有异常，自动插入 alert 和 notification
        -- -------------------------------------------------------------------
        INSERT INTO bill (
        ...
        IF MOD(v_count, p_commit_interval) = 0 THEN
            COMMIT;
            DBMS_OUTPUT.PUT_LINE('  已处理 ' || v_count || ' 个电表，提交');
        END IF;
    ...
    -- 步骤4: 输出执行汇总
    -- -----------------------------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('===== SP1 执行完成 =====');
    DBMS_OUTPUT.PUT_LINE('账期: ' || v_bill_month);
    DBMS_OUTPUT.PUT_LINE('生成账单数: ' || v_count);
    DBMS_OUTPUT.PUT_LINE('========================');

EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('SP1 执行失败，已回滚: ' || SQLERRM);
        RAISE;
