...
CREATE TABLE meter (
    ...
    last_reading       NUMBER(12,2)   DEFAULT NULL,    -- 最近一次读数快照（触发器自动维护）
...
CREATE TABLE alert (
    ...
    alert_level           VARCHAR2(20)    NOT NULL,          -- 告警级别
    ...
    CONSTRAINT ck_alert_level CHECK (alert_level IN ('INFO', 'WARN', 'CRITICAL')),
...
COMMENT ON COLUMN alert.alert_level       IS '告警级别: INFO(信息)/WARN(警告)/CRITICAL(严重)';
